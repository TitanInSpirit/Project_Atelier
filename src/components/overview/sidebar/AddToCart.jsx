    ...
    return (
      <form className='add_to_cart_container' onSubmit={props.handleAddToCart}>
        <div className='size_and_quantity_container'>
          <select
            name='select_size'
            ref={props.addToCartError}
            className='overview_button select_size'
            onChange={props.setCurrentSku}
            required>
            <option value='default' disabled selected required>
              SELECT SIZE
            </option>
            {sizes.length === 0 ? (
              <option value='OOS' id='OOS' key='OOS' disabled selected required>
                'OUT OF STOCK
              </option>
            ) : (
              sizes.map((size) => {
          ...
          <select
            ...
            id='quantity'
            className='overview_button select_quantity'>
            {props.current_size !== 'default' ? null : (
              <option value='default' disabled selected>
                -
              </option>
            )}
            {availabilityArray.length === 0 ? (
              <option value='OOS' id='OOS' key='OOS' disabled selected>
                'OUT OF STOCK
              </option>
            ) : (
              availabilityArray.map((quantity) => {
                return (
                  <option value={quantity} id={quantity} key={quantity}>
                    {quantity}
                  </option>
                );
              })
          ...
          <div className='add_to_cart_favorite_container'>
            {props.current_size === 'default' ? (
              <div className='add_to_cart_flex_container'>
                ...
                <button
                  type='submit'
                  className='add_to_cart_btn overview_button'
                  onClick={() => {
                    setShowModal(true);
                  }}>
                  ADD TO BAG
        ...
        <div className='social_share_container'>
          ...
          />
          <FaPinterest
            className='share_icon pinterest'
            onClick={() => {
