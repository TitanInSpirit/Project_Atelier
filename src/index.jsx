...
class App extends React.Component {
  ...
  handleAddToCart = (event) => {
    ...
    axios.post('/cart', configCart).then((res) => {
      console.log(res);
    });
    event.preventDefault();
  };
  ...
  setCurrentPhoto = (photo) => {
    ...
    if (!this.state.current_style) {
      ...
      this.setState({
        current_photo_index: photo,
        current_photo: this.state.current_style.photos[photo].url,
      });
    }
  };
    ...
    axios
      ...
      .then((response) => {
        this.setState({
          ...
          current_product: response.data[4],
        });
  ...
  setCurrentProduct = (product_id) => {
    ...
    this.state.products.map((productObj) => {
      productObj.id === product_id
        ? this.setState({ current_product: productObj })
        : null;
    });
  };
    ...
    if (!styleId) {
      styles.map((style) => {
        ...
        style['default?']
          ? this.setState({
              current_style: style,
              current_sku: defaultSku,
              current_photo: defaultPhoto,
            })
          : null;
      });
    } else if (styleId) {
      this.state.all_styles.map((style) => {
        ...
        style.style_id === styleId
          ? this.setState({ current_style: style, current_sku: sku })
          : null;
      });
    }
  };
  ...
  getInfo = (productId) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        this.setState({
          total_reviews: response.results.length,
  ...
  getQuestions = (productId) => {
    axios
      ...
      .catch(
        (err) =>
          `Unable to get questions due to following error: ${console.error(
            err.message
          )}`
      );
  };
    ...
    return (
      <div>
        ...
        <Questions
          ...
          getQuestions={this.getAllProducts}
